...
K210_CONVERTER_DOWNLOAD_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), 'ncc_linux_x86_64.tar.xz')
...
class Converter(object):
    def __init__(self,
                 ...
                 backend=None,
                 dataset_path=None,
                 ncc_path=K210_CONVERTER_PATH):
        if 'tflite' in converter_type:
            print('Tflite Converter ready')

        if 'k210' in converter_type:
            if os.path.exists(ncc_path):
                print('Buildin K210 Converter ready')
            elif 0 == os.system("ncc"):
                print('K210 Converter in system PATH')
                ncc_path = "ncc"
            else:
                print('Downloading K210 Converter')
                _path = tf.keras.utils.get_file(K210_CONVERTER_DOWNLOAD_PATH,
                                                NNCASE_DOWNLOAD_URL)
                print(_path)
                tar_file = tarfile.open(K210_CONVERTER_DOWNLOAD_PATH)
                tar_file.extractall(
                    os.path.join(os.path.dirname(__file__), "ncc"))
                tar_file.close()
                os.chmod(ncc_path, 0o775)
    ...
    def edgetpu_dataset_gen(self):
        ...
        backend = create_feature_extractor(
            self._backend, [self._img_size[0], self._img_size[1]])
        image_search = lambda ext: glob.glob(self._dataset_path + ext,
                                             recursive=True)
    ...
    def convert_edgetpu(self, model_path):
        ...
        print(output_path)
        cmd = "edgetpu_compiler --out_dir {} {}".format(
    ...
    def convert_onnx(self, model, model_layers):
        ...
        with tf.Graph().as_default() as tf_graph:
            ...
            onnx_graph = tf2onnx.tfonnx.process_tf_graph(
                tf_graph,
                input_names=input_node_names_onnx,
                output_names=output_node_names_onnx)
    ...
    def convert_tflite(self, model, model_layers, target=None):
        ...
        if target == 'k210':
            if model_type == 'segnet':
                print(
                    "Converting to tflite with old converter for K210 Segnet")
        ...
        open(os.path.join(self.model_path.split(".")[0] + '.tflite'),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Keras model conversion to .kmodel, .tflite, or .onnx")
    parser.add_argument("--model_path",
                        "-m",
                        type=str,
                        required=True,
                        help="path to keras model")
    parser.add_argument("--converter_type",
                        type=str,
                        default='k210',
                        help="batch size")
