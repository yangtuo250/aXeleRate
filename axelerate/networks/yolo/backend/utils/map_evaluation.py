...
class MapEvaluation(tensorflow.keras.callbacks.Callback):
    ...
    def __init__(self,
        ...
        self._iou_threshold = yolo.yolo_params.iou_thresh
        self._score_threshold = yolo.yolo_params.obj_thresh
        ...
        if not isinstance(self._tensorboard, tensorflow.keras.callbacks.
                          TensorBoard) and self._tensorboard is not None:
            raise ValueError(
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if epoch == 0:
            ...
            self.model.save(self._save_name.format("_epoch1_"),
                            overwrite=True,
        ...
        if (epoch + 1) % self._period == 0 and self._period != 0:
            ...
            for label, average_precision in average_precisions.items():
                print(self._yolo._labels[label],
                      '{:.4f}'.format(average_precision))
            print('mAP: {:.4f}'.format(_map))

            if self._save_name:
                if _map > self.bestMap:
                    print("mAP improved from {} to {}".format(self.bestMap, _map))
                    self.bestMap = _map
                    if self._save_best:
                        print("Saving best model to {}".format(self._save_name.format("_best_")))
                        self.model.save(self._save_name.format("_best_"), overwrite=True, include_optimizer=False)
                else:
                    print("mAP did not improved from {}".format(self.bestMap))
                print("mAP {}, saving model to {}.".format(_map, self._save_name.format("_"+"epoch"+str(epoch)+"_"+str(round(_map, 4))+"_")))
                self.model.save(self._save_name.format("_"+"epoch"+str(epoch)+"_"+str(round(_map, 4))+"_"),
                                overwrite=True,
                                include_optimizer=False)
            else:
                print("Model not save for save_name not defined")
    ...
    def _calc_avg_precisions(self):
        ...
        all_detections = [[
            None for _ in range(len(self._yolo._labels))
        ...
        for i in range(len(self._generator)):
            ...
            for j in range(len(annotations)):
                ...
                _, pred_boxes, probs = self._yolo.predict(
                    input_image,
                    height,
                    width,
                ...
                for label in range(len(self._yolo._labels)):
                    all_detections[counter][label] = pred_boxes[pred_labels ==
                                                                label, :]

                # copy ground truth to all_annotations
                for label in range(len(self._yolo._labels)):
                    all_annotations[counter][label] = annotations[j][
                        annotations[j][:, 4] == label, :4].copy()
                counter += 1
